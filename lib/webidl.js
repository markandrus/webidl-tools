...
function addOptions(program, o, w, options) {
  ...
  return program
    ...
    .option('--merge', util.wrap(
      ...
      options.merge)
    .option('--no-duplicates', util.wrap(
...
/**
 * Merge partial definitions, dictionaries, etc., if specified.
 * @param {Map<string, Array<object>>} definitionsByName
 * @param {object} options
 * @returns {Array<object>}
 */
function maybeMergeDefinitionsByName(definitionsByName, options) {
  if (options.merge || options.duplicates === false) {
    return mergeDefinitionsByName(definitionsByName, options).reduce((mergedDefinitionsByName, mergedDefinition) => {
      mergedDefinitionsByName.set(mergedDefinition.name, [mergedDefinition]);
      return mergedDefinitionsByName;
    }, new Map());
  }
  return definitionsByName;
}

exports.maybeMergeDefinitionsByName = maybeMergeDefinitionsByName;
...
function mergeDefinitionsByName(definitionsByName, options) {
  return Array.from(definitionsByName.values()).map(definitions => {
    if (definitions.length > 1 && options.duplicates === false) {
