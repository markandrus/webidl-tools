...
var _defaultJavaScriptRenamings = {
  DOMString: 'string',
  DOMTimeStamp: 'number',
  sequence: 'Array',
...
var defaultJavaScriptRenamings = [];
for (var regex in _defaultJavaScriptRenamings) {
  defaultJavaScriptRenamings.push([new RegExp('^' + regex + '$'),
    _defaultJavaScriptRenamings[regex]]);
}

var _defaultCPPRenamings = {
...
var defaultCPPRenamings = [];
for (var regex in _defaultCPPRenamings) {
  defaultCPPRenamings.push([new RegExp('^' + regex + '$'),
    _defaultCPPRenamings[regex]]);
}

/**
 * Add default JavaScript renamings.
 * @param {object} options
 * @returns {object}
 */
function addDefaultJavaScriptRenamings(options) {
  options.rename = defaultJavaScriptRenamings.concat(options.rename);
  return options;
}

exports.addDefaultJavaScriptRenamings = addDefaultJavaScriptRenamings;

/**
 * Add default CPP renamings.
 * @param {object} options
 * @returns {object}
 */
function addDefaultCPPRenamings(options) {
  options.rename = defaultCPPRenamings.concat(options.rename);
  return options;
}

exports.addDefaultCPPRenamings = addDefaultCPPRenamings;
...
function loadTemplate(template, extra) {
  ...
  return function template(data) {
    var merged = Object.assign({
      ...
      cppEnum: cppEnum,
      template: template => loadTemplate(template, data)
    }, data, extra);
